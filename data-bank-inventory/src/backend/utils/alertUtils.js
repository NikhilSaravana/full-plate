...
export function generateCategoryAlerts({ currentInventory, memoizedTotalInventory, outgoingMetrics, targetCapacity, t }) {
  const alerts = [];
  ...
  Object.entries(currentInventory).forEach(([category, weight]) => {
    ...
    if (percentage < 5 && total > 0) {
      alerts.push({
        ...
        message: t ? t('alerts.inventory-critically-low', { category, percentage: percentage.toFixed(1) }) : `${category} inventory critically low (${percentage.toFixed(1)}%)`,
        action: t ? t('alerts.consider-immediate-restocking') : 'Consider immediate restocking',
        priority: 'high',
        source: 'category'
      });
      ...
      alerts.push({
        type: 'WARNING',
        category: 'LOW_INVENTORY',
        message: t ? t('alerts.inventory-low', { category, percentage: percentage.toFixed(1) }) : `${category} inventory low (${percentage.toFixed(1)}%)`,
        action: t ? t('alerts.plan-restocking-soon') : 'Plan for restocking soon',
        priority: 'medium',
        source: 'category'
      });
    }
  ...
  if (compliantCategories < 3) {
    alerts.push({
      ...
      message: t ? t('alerts.myplate-imbalance', { compliantCategories }) : `Only ${compliantCategories}/5 MyPlate categories are balanced`,
      action: t ? t('alerts.review-distribution-targets') : 'Review distribution targets',
      priority: 'medium',
      source: 'category'
    });
  }
...
export function generateItemAlerts(detailedInventory, UnitConverters, t) {
  const newAlerts = [];
...
export function getCombinedAlerts({ currentInventory, memoizedTotalInventory, outgoingMetrics, detailedInventory, UnitConverters, targetCapacity, t }) {
  const categoryAlerts = generateCategoryAlerts({ currentInventory, memoizedTotalInventory, outgoingMetrics, targetCapacity, t });
  const itemAlerts = generateItemAlerts(detailedInventory, UnitConverters, t);
